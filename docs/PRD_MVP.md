...
- 操作：
  ...
  - 导出/分享：保存到相册或系统分享（MVP占位）
  - 失败重试：渲染失败时显示“再试一次”（仅重试渲染，不重复创建）
...
### 9.1 位置模糊化策略（MVP）
- 默认不展示精确坐标，仅展示 zone_name（如“太古里附近”）
- 允许设置 radius_m（模糊半径，默认 3000）
- geo.hidden=true 时，前端不显示任何位置信息
